...
func (s *state) startServices(src string, rt runtimeTree) {
	step("Starting the services")
	env := rt.pathWith(s.serviceEnv())
	dir := src
	if rt.present() {
		// The runtime directory, not the source tree: under a bundled install there is no source
		// tree at all, and a relative path a service resolves against its working directory has to
	...
	for _, svc := range services {
		...
		program, args := "java", []string{}
		if rt.present() {
			program, args = rt.java(), rt.javaArgs(svc.Name)
		} else {
			jar := jarFor(src, svc.Name)
			if jar == "" {
				fail("no jar for %s — the build did not produce one", svc.Name)
			}
			args = []string{"-jar", jar, "--spring.profiles.active=dev"}
		}
		pid, err := spawn(filepath.Join(logDir(), svc.Name+".log"), dir, env, program, args...)
		if err != nil {
...
func (s *state) startWeb(src string, rt runtimeTree) {
	if portBusy(webPort) {
	...
	program, args := nodeTool("npx"), []string{"next", "start", "--port", fmt.Sprint(webPort)}

	if rt.present() {
		// Next.js standalone output: one server.js and the minimum node_modules it actually
		// imports, started by the Node binary directly. `output: "standalone"` has been set in
		// web/next.config.ts all along and nothing used it — `npx next start` runs the ordinary
		// build, which needs the whole 614 MB node_modules tree present at run time. Reading the
		// standalone entry point instead is most of the reason the web half of this payload is
		// 67 MB rather than most of a gigabyte.
		dir = rt.webDir()
		program, args = rt.node(), []string{rt.webEntry()}
		env = append(rt.pathWith(env),
	...
	pid, err := spawn(filepath.Join(logDir(), "web.log"), dir, env, program, args...)
	if err != nil {
...
// startAI starts the clinical decision-support service, which until this bundle was never started
// by anything.
//
// It is optional by design — `hms.ai.enabled` exists precisely so the platform runs without it, and
// every consumer has a documented fallback: no-show risk falls open, note summarisation drops to an
// extractive summariser. What was not by design is the state the installer left it in. The flag
// defaults to **true**, nothing set it, and nothing listened on 8000, so scheduling-service opened a
// connection to a closed port on every booking and waited out the circuit breaker before falling
// open. Correct behaviour, arrived at the slow way, on every appointment anybody booked.
//
// So there are two fixes here and the second one matters even when this returns early: the service
// is started when the payload carries it, and HMS_AI_ENABLED is stated either way (state.go).
func (s *state) startAI(rt runtimeTree) {
	if !rt.present() || !rt.hasAI() {
		if s.AI {
			warn("the AI service is enabled but not in this payload")
		}
		return
	}
	if portBusy(aiPort) {
		ok("%-22s already on %d", aiService.Name, aiPort)
		s.Ports[aiService.Name] = aiPort
		return
	}
	step("Starting the AI service")
	env := rt.pathWith(s.serviceEnv())
	// The embeddable Python runs uvicorn as a module rather than through a console script: the
	// embeddable distribution has no Scripts directory and no uvicorn.exe, and -m is the entry
	// point that does not depend on one having been generated by an installer that never ran here.
	pid, err := spawn(filepath.Join(logDir(), aiService.Name+".log"), rt.aiDir(), env,
		rt.python(), "-m", "uvicorn", "app.main:app",
		"--host", "127.0.0.1", "--port", fmt.Sprint(aiPort))
	if err != nil {
		warn("could not start the AI service: %v", err)
		return
	}
	s.Pids[aiService.Name] = pid
	s.Ports[aiService.Name] = aiPort
	s.save()

	if !waitHTTP(fmt.Sprintf("http://127.0.0.1:%d/actuator/health", aiPort), 90*time.Second) {
		// A warning and not a failure, and the distinction is the point of the flag: the platform
		// is built to run without this service, so refusing to install because the optional half
		// did not start would be the installer inventing a hard dependency the code does not have.
		bad("%-22s did not come up", aiService.Name)
		explainLog(filepath.Join(logDir(), aiService.Name+".log"))
		warn("continuing without it — MedSync runs with HMS_AI_ENABLED=false")
		_ = terminate(pid)
		delete(s.Pids, aiService.Name)
		delete(s.Ports, aiService.Name)
		s.AI = false
		s.save()
		return
	}
	ok("%-22s pid %-7d port %d", aiService.Name, pid, aiPort)
}

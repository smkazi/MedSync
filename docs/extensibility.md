...
| **How long break-glass cover lasts, and how much of a reason it takes** | `hms.care-team.break-glass-ttl`, `hms.care-team.reason-min-length` | configuration: twelve hours and twenty characters. The first is one shift, because covering somebody else's patient for an evening should not become standing access for ever; the second is a floor rather than a rule, because "cover" is what a free-text box collects when it does not ask for a sentence |
| **What a day means** | `HMS_ZONE`, overridden per service by `hms.scheduling.zone`, `hms.billing.zone`, `hms.audit.zone`, `hms.interop.zone`, `hms.immunisation.zone` | configuration. "The 14th" means the hospital's 14th, not the container's. Five things bound a day — clinic slot generation and the OPD board, the billing day book, the audit report, the disclosure register and the immunisation register — and each defaults from the single `HMS_ZONE`, so a deployment sets one variable and they cannot disagree. That promise was not true when it was written: scheduling read a different variable, `HMS_TIMEZONE`, and defaulted to UTC, so a compose deployment ran it a day apart from the other three. The row is the rule; scheduling is now on it |
...
| **The examination catalogue** | `imaging.imaging_procedures` | a migration. Configuration in the sense that matters — a department that starts doing MRI knees adds a row, and the ordering screen offers whatever it finds — but there is no write endpoint yet, so today it is a migration and the catalogue screen says read-only rather than fronting a form that would 405. Named in the README's gaps rather than left as a surprise |
| **Where radiology images are kept** | `hms.imaging.storage-dir` | configuration, and **unset by default**. Unset means a study is registered in full from its header and the screens say no archive is configured; set it and the bytes are written under it and the URI is recorded. The same posture as the notification channels and the ABDM gateway: a working default, one adapter, and no pretence about the unwired half — this is a RIS and not a PACS |
| **The accession prefix** | `hms.imaging.accession-prefix`, default `IMG` | configuration. The number itself comes from a sequence, so two concurrent orders cannot share one; the prefix is what a department's paperwork and its modalities already say, and it is the one part of the identifier a deployment has an opinion about |
| **The vaccine catalogue, and what each product contains** | `immunisation.antigens`, `.vaccine_products`, `.vaccine_product_antigens` | `POST /vaccines/antigens`, `POST /vaccines/products`, `PATCH /vaccines/products/{code}` — administrator-only. The contents list is written once and never edited, the same rule `pharmacy.formulary_ingredients` states one department along and for a sharper reason: a child recorded as having had PENTA in 2024 had whatever PENTA contained in 2024, and a row edited in 2026 must not retrospectively give them a Hib dose they did not receive |
| **The immunisation schedule** | `immunisation.immunisation_schedules`, `.schedule_doses` | a migration today, and configuration in the sense that matters: a deployment running a different national schedule inserts its rows, points `hms.immunisation.default-schedule` at them and changes no code. Every number is **days from date of birth** — "10 weeks" is 70 days and "2 months" is 59, 60, 61 or 62, so a schedule written in months is up to four days wrong for every child in the district at once, in the same direction, silently. Written in **antigens** rather than products, so switching brand neither rewrites the schedule nor stops already-recorded doses counting. Administrator-only, because editing one row moves the due date for every child at once |
| **How late is overdue** | `immunisation.schedule_doses.grace_days`, per dose | configuration, and per-row because "overdue" is a local judgement: a clinic that calls at a fortnight and one that calls at three months are both running the same schedule, and a number hard-coded in Java would make the word mean something nobody in the building could change. The *ages* are not local in the same way — they come from a published schedule — which is the NEWS2 line drawn one module along, and the reason the escalation policy is a row while the bands are not |
| **How many children one calling list returns** | `hms.patient.cohort-max-rows`, 2000 | configuration, and the cap announces itself on the response. Two thousand is roughly a fortnight of births in a large district, which is the unit an immunisation clinic works in; the answer to the cap biting is a narrower birth range rather than a page. Stated rather than silent because the README's findings record what a quietly capped pick-list cost once, and a calling list truncated in silence drops exactly the children nobody then telephones |
| **Quality measures** and what each requires | `immunisation.quality_measures`, `.quality_measure_antigens` | a migration or an INSERT. **A second coverage measure is rows**: the age it asks about, the antigens and dose counts it wants, its steward, its specification version and whether it counts recollected dates are all columns, and `MeasureApiIntegrationTest.aSecondMeasureIsRows` asserts one inserted at runtime computes a correct rate with no restart. What is *not* configuration is `kind` — see the row below |
| **Notifiable conditions** | `scheduling.notifiable_conditions` | a migration or an INSERT. **One row per ICD-10 code, never a prefix** — a prefix widens invisibly (`A0` sweeps cholera through typhoid into amoebiasis and one line of a statutory return, and changes on its own the day somebody adds a code under it) and could not be an equality join, so `idx_diagnoses_code` would go unused. `notify_within_hours` is a number rather than an urgency enum because how many hours *is* the whole of the behaviour; it is recorded and enforced by nothing, because this platform has no outbound channel to an authority and a countdown it could not act on would be a promise nothing keeps |
| **How small a count may be before it is withheld** | `hms.surveillance.small-cell-threshold`, **0 — off** | configuration, and off is the shipped default rather than a placeholder. A statutory return needs exact counts: a district filing "fewer than five" cannot be aggregated upward by whoever receives it, and a threshold applied silently would understate an outbreak. The mechanism exists because the risk is real — a rare condition in a small population is re-identifying by arithmetic — and a deployment publishing more widely than to its own authority turns it on. When it bites, the count is withheld as null rather than rendered as zero and the report says so, because a suppressed count and no cases are different facts |
| **Who the line list is notified to** | `hms.surveillance.authority`, default *District public health authority* | configuration, and deliberately not a request parameter. `recipient` is the column a patient reads when they ask who has seen their record, and one typed into a form per request could be filled with anything — including a name that makes an unlawful disclosure read as a statutory one. A deployment notifies the authority it is licensed under; that is a property of the deployment and changes about as often as the licence does |
| **Whether a line list can be produced at all** | not configurable — `hms.interop.base-url` must be reachable | code, and stated here because it looks like a knob and is not. The disclosure register is written before a byte of the file exists, and an unreachable register answers 503 with no file rather than degrading to a file nobody recorded. There is deliberately no property that turns that off: a switch marked *produce the list without registering it* would be a switch for the one outcome this module is built to refuse |
...
| What a wristband carries | In code, in `WristbandRenderer`, and not a template. Two fields decide whether it works and neither is a preference: the barcode payload is the MRN because that is what `AdministrationService` compares a scan against, and the printed line is the MRN again because a scanner that will not read leaves somebody to type it. A configurable layout would let a deployment put an internal id in the symbol — a band that scans cleanly and refuses every dose — or drop the human-readable line, which fails only in the situation the band exists for. Deliberately absent: an allergy marker, since this platform cannot guarantee a colour printer and a warning that is sometimes there trains people to read a band for the absence of one. |
...
| **What the portal refuses to show** | Unverified laboratory results and unsigned notes. Both are filtered in code, in the service that owns the record, and neither is a setting: a provisional value published early makes the patient the first reader of a number that may be wrong, and a draft shown to its subject becomes a statement its author never made. A deployment that wanted either would be changing what "released" and "signed" mean. |
| **The portal booking horizon** | `hms.portal.booking-horizon-days`, default 60. Configuration, because how far ahead a clinic publishes its rota is a deployment's decision and nothing behaves differently at 30 or at 90. The refusal names the number, so a patient is told to telephone rather than left guessing. |
...
| **The modality vocabulary** (`CR`, `CT`, `MR`, `US`, `MG`, …) | Not a list this platform owns. It is DICOM's, written into every image by the machine that made it, and the platform reads it out of the header rather than choosing it — so there is nothing to configure and nothing to validate against: an unrecognised modality is recorded as what the file said, because the file is the evidence. The worklist filter offers the five a general hospital runs and the API accepts any string, which is the honest way round: a filter that refused a modality the department owns would hide its work. |
| `ImagingOrderStatus`, `ImagingReportStatus` | The same separation of duties the laboratory's statuses encode, one department along. ACQUIRED is set by an image arriving and REPORTED by a signature, so neither is a caller's to set; and `DRAFT` → `SIGNED` → `AMENDED` is one-way, with `chk_report_amended` insisting that an amendment carries the text it superseded. A configurable list would let somebody add a state between signed and released, which is exactly the state this design refuses to have. |
| **What a study is matched on** | The accession number, and nothing else. Not configurable, and this is the sharpest example in the document of why: the alternative fields are all *available* — a DICOM header carries a patient name, a patient id, a birth date — and they are whatever was typed at a modality console. Matching on them would file a study against the wrong visit the first time somebody was scanned twice in a day, and against the wrong patient the first time a name was mistyped. The accession number is the one field the worklist puts into the machine and the machine writes back, so it is the only one that is evidence. A deployment that could switch this would be switching off the guarantee. |
...
| `ThreadStatus` (secure messaging) | Three states, and none of them is a caller's to set: OPEN and ANSWERED are derived from who wrote last, and CLOSED is staff-only and final. A settable status would be set to ANSWERED by whoever wanted the queue to look shorter, and a fourth state would need a rule about who may move into it — which is the behaviour, not the vocabulary. |
| `AuthorKind` (secure messaging) | Two sides, because there are two: the patient and the hospital. It decides who may read the thread and how its status moves, so it is not a display label, and deriving it from an author's name is how a member of staff called "Patient" would break a conversation. |
| **Whose record a portal session may read** | The `patient_id` claim on the access token, written at enrolment and never moved. Not configuration and not a request parameter: `CurrentUser.requirePatientId()` is the only source, so there is no field to tamper with and no per-deployment way to widen it. A deployment that wanted a carer to read somebody else's record would need a proxy-access model with its own consent, which is a feature rather than a setting. |
| `ImmunisationSource` | Three values, and each decides which columns a dose must carry: ADMINISTERED_HERE demands a lot, a route, a site and a clinician; the two historical values forbid a lot and demand a sentence saying what was seen. The database enforces it as a biconditional, so a fourth value would be a row with no rule about what it must carry — a gap in the record wearing a source's clothes. |
| `DueStatus` | Computed, never stored, and every value carries behaviour: OVERDUE is what a calling list is made of, EXEMPT suppresses a call, NO_LONGER_GIVEN says a window closed and nothing further is due. A configurable list would let somebody add a state the calling list does not know how to filter on. Note the split it encodes: a MEDICAL exemption makes a dose EXEMPT and a REFUSED one deliberately does not — a family who declined last year is exactly who a clinic may want to speak to again — so the row stays and carries `refusalRecorded` instead. |
| **A quality measure's `kind`** | The NEWS2 argument with the sign reversed, and the pair is worth reading together. There, a deployment able to edit the bands could publish a number it calls NEWS2 which is not NEWS2 — a wrong answer carrying a standard's authority. Here, a deployment able to add a `kind` with no calculator behind it could publish a rate that is not a rate at all: a percentage rendered from two zeroes. **The first is a wrong answer; the second is an answer with nothing behind it, which is worse, because the first can at least be checked.** So `kind` carries a database CHECK naming exactly the calculators that exist, and a row asking a question nothing can answer is refused at the INSERT rather than rendered as a number. Adding a kind is a code change and a migration, in that order. |
| **What counts as a dose** | `ImmunisationScheduleCalculator`, and not a table. Whether a dose given too early or too soon after the previous one advances the series is the rule the whole register rests on, and a deployment able to edit it could report a district as covered by relaxing the intervals. What *is* a table is the schedule those rules are applied to — the ages, the intervals and the grace periods — which is the same line NEWS2 draws: the arithmetic is code, the numbers it runs on are rows. And there is no `ZoneId` anywhere in it, because an interval between two dates has none: a schedule means the same thing in a UTC container and in an IST one. |

...
public interface LabResultRepository extends JpaRepository<LabResult, UUID> {
    ...
    List<LabResult> findByOrderIdOrderByParameter(UUID orderId);

    Optional<LabResult> findByOrderIdAndParameter(UUID orderId, String parameter);

    long countByOrderIdAndStatus(UUID orderId, LabEnums.ResultStatus status);
